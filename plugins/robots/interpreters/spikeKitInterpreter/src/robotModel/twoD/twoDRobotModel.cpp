...
robotParts::Device *TwoDRobotModel::createDevice(PortInfo const &port, DeviceInfo const &deviceInfo)
{
	...
	if (deviceInfo.isA<spike::robotModel::parts::SpikeLed>()) {
...
{
	const QString key = "spikeRobot2DImage";
	const QString hackDefaultPath = "./images/spike-robot.png";
	...
	return QFile::exists(settingsPath) ? settingsPath : ":/spike/interpreter/images/spike-robot.png";
}
...
QString TwoDRobotModel::sensorImagePath(const DeviceInfo &deviceType) const
{
	if (deviceType.isA<kitBase::robotModel::robotParts::TouchSensor>()) {
		return ":/spike/interpreter/images/spike-touch.png";
	} else if (deviceType.isA<kitBase::robotModel::robotParts::RangeSensor>()) {
		return ":/spike/interpreter/images/spike-sonar.png";
	} else {
		return QString();
	}
}
