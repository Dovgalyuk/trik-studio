...
using namespace spike::robotModel::real;
...
void Button::read()
{
	QByteArray command = SpikeDirectCommand::formCommand(13, 4, 1, 0
			, enums::commandType::CommandTypeEnum::DIRECT_COMMAND_REPLY);
	int index = 7;
	SpikeDirectCommand::addOpcode(enums::opcode::OpcodeEnum::UI_BUTTON_PRESSED, command, index);
	SpikeDirectCommand::addByteParameter(parsePort(port().name()), command, index);
	SpikeDirectCommand::addGlobalIndex(0, command, index);
