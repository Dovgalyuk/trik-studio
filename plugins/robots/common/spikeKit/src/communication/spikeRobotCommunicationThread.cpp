...
static constexpr uchar SPIKE_SYSTEM_COMMAND_REPLY =             0x01;    //  System command, reply required
static constexpr uchar SPIKE_SYSTEM_COMMAND_NO_REPLY =          0x81;    //  System command, reply not required
static constexpr uchar SPIKE_BEGIN_DOWNLOAD =                   0x92;    //  Begin file down load
static constexpr uchar SPIKE_CONTINUE_DOWNLOAD =                0x93;    //  Continue file down load
static constexpr uchar SPIKE_DELETE_FILE =                      0x9C;    //  Remove file
static constexpr uchar SPIKE_SYSTEM_REPLY =                     0x03;    //  System command reply
static constexpr uchar SPIKE_SYSTEM_REPLY_ERROR =               0x05;    //  System command reply error
static constexpr uchar SPIKE_DELETE_FILE_RESPONSE_SIZE =        8;
static constexpr uchar SPIKE_BEGIN_DOWNLOAD_RESPONSE_SIZE =     8;
static constexpr uchar SPIKE_CONTINUE_DOWNLOAD_RESPONSE_SIZE =  8;
static constexpr uchar SPIKE_SYSTEM_COMMAND_REPLY_SUCCESS =  0x00;
static constexpr uchar SPIKE_CONTINUE_DOWNLAD_STATUS_EOF =      0x08;

using namespace spike::communication;

SpikeRobotCommunicationThread::~SpikeRobotCommunicationThread()
{
}
...
QString SpikeRobotCommunicationThread::uploadFile(const QString &sourceFile, const QString &targetDir)
{
	...
	if (filenameLength > SPIKE_MAX_ALLOWED_FILENAME_LENGTH) {
		Q_EMIT errorOccured(tr("SPIKE limits filename length to %1 characters, "
				"but you have %2, please, rename your project.").arg(SPIKE_MAX_ALLOWED_FILENAME_LENGTH)
				.arg(filenameLength));
		return QString();
	}
...
//	QByteArray commandDeleteResponse = receive(DELETE_FILE_RESPONSE_SIZE);
	...
	commandBegin[4] = charOf(SPIKE_SYSTEM_COMMAND_REPLY);
	commandBegin[5] = charOf(SPIKE_BEGIN_DOWNLOAD);
	...
	if (!send1(commandBegin)) {
		QLOG_ERROR() << "SPIKEUSB" << "Failed to start program upload to robot";
		return QString();
	}

	QByteArray commandBeginResponse = receive(SPIKE_BEGIN_DOWNLOAD_RESPONSE_SIZE);

	if (commandBeginResponse.at(4) == SPIKE_SYSTEM_REPLY_ERROR) {
		QLOG_ERROR() << "SPIKEUSB"
		...
		if (commandBeginResponse.size() > 7) {
			QLOG_INFO() << "SPIKEUSB" << "Reply additional:"
	...
	while (sizeSent < data.size()) {
		...
		commandContinue[4] = charOf(SPIKE_SYSTEM_COMMAND_REPLY);
		commandContinue[5] = charOf(SPIKE_CONTINUE_DOWNLOAD);
		...
		if (!send1(commandContinue)) {
			QLOG_ERROR() << "SPIKEUSB" << QString("Failed to send program data to robot bytes %1..%2")
					.arg(sizeSent-sizeToSend).arg(sizeSent-1);
			return QString();
		}

		QByteArray commandContinueResponse = receive(SPIKE_CONTINUE_DOWNLOAD_RESPONSE_SIZE);
		if (commandContinueResponse.at(4) == SPIKE_SYSTEM_REPLY_ERROR) {
			QLOG_ERROR() << "SPIKEUSB"
...
bool SpikeRobotCommunicationThread::runProgram(const QString &pathOnRobot)
{
	QByteArray command = SpikeDirectCommand::formCommand(21 + pathOnRobot.size(), ++mMessageCounter, 0x08, 0
...
void SpikeRobotCommunicationThread::stopProgram()
{
	QByteArray command = SpikeDirectCommand::formCommand(9, ++mMessageCounter, 0, 0
			, enums::commandType::CommandTypeEnum::DIRECT_COMMAND_NO_REPLY);
	command[7] = 0x02;  // opPROGRAM_STOP Opcode
	command[8] = 0x01;  // LC0(USER_SLOT), User slot = 1 (program slot)
	if (!send1(command)) {
		QLOG_ERROR() << "SPIKEUSB" << "Failed to stop program";
	}
}
