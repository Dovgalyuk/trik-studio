...
static const int SPIKE_VID = 0x0694;
static const int SPIKE_PID = 0x0005;
static const int SPIKE_USB_TIMEOUT = 2000;
static const int SPIKE_INTERFACE_NUMBER = 0;
static const int SPIKE_CONFIGURATION_NB = 1;
static const int SPIKE_EP_OUT = 0x01;
static const int SPIKE_EP_IN = 0x81;
static const int SPIKE_PACKET_SIZE = 0x400;
...
{
	if (mHandle) {
		QLOG_INFO() << "SPIKEUSB" << "Handle reused";
	...
	mHandle = hid_open(SPIKE_VID, SPIKE_PID, nullptr);
	if (!mHandle) {
		QLOG_ERROR() << "hid_open failed for "
				<< QString::number(SPIKE_VID, 16) << QString::number(SPIKE_PID, 16);
		Q_EMIT connected(false, tr("Cannot find SPIKE device. Check robot connected and turned on and try again."));
		return false;
	}
...
void UsbRobotCommunicationThread::checkForConnection()
{
	...
	if (!send1(command)) {
		QLOG_ERROR() << "SPIKEUSB" << "Connection lost";
		disconnect();
	}
}
...
bool UsbRobotCommunicationThread::send1(const QByteArray &buf) const
{
	...
	if (!ok) {
		QLOG_ERROR() << "SPIKEUSB" << "Failed hid_write with" << n
...
{
	// It's strange, but regardless of kind of response, it must have size = 1024 (SPIKE_PACKET_SIZE)
	// If we set another size(For example, response[10]), program will throw error.
	uchar response[SPIKE_PACKET_SIZE] {0, };
	size_t bufSize = sizeof(response)/sizeof (response[0]);
//	response[0] = 0;
//	response[1] = SPIKE_EP_IN;
//	auto n = hid_write(mHandle, response, bufSize);
//	if (n != bufSize) {
//		QLOG_ERROR() << "SPIKEUSB" << "Failed hid_write with errno =" << errno;
//	}

	auto n = hid_read(mHandle, response, bufSize);
	if ( n <= 0) {
		QLOG_ERROR() << "SPIKEUSB" << "Failed hid_read with" << n << ", errno = " << errno;
	}
	auto resultSize = qMin(SPIKE_PACKET_SIZE, size);
