...
bool BluetoothRobotCommunicationThread::connect()
{
	...
	const QString portName = qReal::SettingsManager::value("SpikeBluetoothPortName").toString();
...
{
	QByteArray command = SpikeDirectCommand::formCommand(10, 0, 0, 0
			, enums::commandType::CommandTypeEnum::DIRECT_COMMAND_REPLY);
	int index = 7;
	SpikeDirectCommand::addOpcode(enums::opcode::OpcodeEnum::KEEP_ALIVE, command, index);
	SpikeDirectCommand::addByteParameter(10, command, index); // 10 - Number of minutes before entering sleep mode.
	if (!send1(command)) {
