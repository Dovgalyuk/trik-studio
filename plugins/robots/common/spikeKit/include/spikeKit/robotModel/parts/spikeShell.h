...
/// Executes shell commands on SPIKE controller
class SpikeShell : public kitBase::robotModel::robotParts::Shell
{
	Q_OBJECT

public:
	/// Constructor, takes device type info and port on which this motor is configured.
	SpikeShell(const kitBase::robotModel::DeviceInfo &info
			, const kitBase::robotModel::PortInfo &port);
	virtual ~SpikeShell();

	/// Run current command in the spike
