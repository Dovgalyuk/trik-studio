...
/// Block factory that creates blocks for SPIKE kit.
class SpikeBlocksFactory : public kitBase::blocksBase::CommonBlocksFactory
{
public:
	/// Constructor.
	/// @param interpretedModels - a list of robot ids which are handled by interpreter, so parallel tasks
	///        may be enabled for them.
	SpikeBlocksFactory(const QStringList &interpretedModels);
