...
{
	auto mGeneratorFactory = dynamic_cast<SpikeGeneratorFactory *>(parent);
	...
	if (!mGeneratorFactory->mailboxes().tryRegisterReadMailbox(mailboxName, type)) {
		mGeneratorFactory->reportError(
					SpikeGeneratorFactory::tr("There is already mailbox with same name, but different msg type") , mId);
	}

	if (mGeneratorFactory->mailboxes().mailboxesCount() >= 30) {
		mGeneratorFactory->reportError(SpikeGeneratorFactory::tr("There are too many mailboxes, max size is 30") , mId);
	}
