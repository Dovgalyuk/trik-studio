...
#include "spikeDeviceVariables.h"

#include <QtCore/QMetaClassInfo>

using namespace spike::parts;

QString SpikeDeviceVariables::variableTemplatePath(const kitBase::robotModel::DeviceInfo &device
		, const kitBase::robotModel::PortInfo &port) const
{
	if (device.name() == "spikeLineSensor"
			|| device.name() == "spikeObjectSensor"
			|| device.name() == "spikeColorSensor")
	{
