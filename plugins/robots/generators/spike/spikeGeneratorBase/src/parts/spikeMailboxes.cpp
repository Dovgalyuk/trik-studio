...
using namespace spike::parts;

const QMap<QString, QString> SPIKE_TYPE_ID = {{"int", "2"}, {"bool", "0"}, {"string", "4"}, {"float", "3"}};
const QMap<QString, QString> SPIKE_TYPE_LENGTH = {{"int", "4"}, {"bool", "1"}, {"string", "255"}, {"float", "4"}};
...
QString parts::Mailboxes::generateOpening() const
{
	...
	while (iterator.hasNext()) {
		...
		res += tmpTemplate.replace("@@MAILBOX_NAME@@", iterator.key())
				...
				.replace("@@TYPE@@", SPIKE_TYPE_ID[iterator.value()]);
		res += "\n";
	}
...
{
	return SPIKE_TYPE_LENGTH[typeName];
}

QString parts::Mailboxes::typeToSpikeType(const QString &typeName) const
{
	return SPIKE_TYPE_ID[typeName];
}
