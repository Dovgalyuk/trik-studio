...
/// Represents type of variables available in SPIKE bytecode.
enum class SpikePythonType
{
	/// 8-bit integer
	data8 = 0
	/// 16-bit integer
...
class SpikeLuaPrinter : public qrtext::lua::LuaAstVisitorInterface, public generatorBase::TemplateParametrizedEntity
{
public:
	SpikeLuaPrinter(const QStringList &pathsToTemplates
			, const qrtext::LanguageToolboxInterface &textLanguage
			, generatorBase::parts::Variables &variables);

	~SpikeLuaPrinter() override;
...
private:
	...
	void processBinary(const QSharedPointer<qrtext::core::ast::BinaryOperator> &node, SpikePythonType operandsType
	...
	QString castTo(SpikePythonType targetType, const QSharedPointer<qrtext::lua::ast::Node> &node);
	SpikePythonType moreCommon(SpikePythonType type1, SpikePythonType type2) const;

	QStringList addSuffix(const QStringList &list);
	SpikePythonType toSpikeType(const QSharedPointer<qrtext::core::types::TypeExpression> &type);
	SpikePythonType typeOf(const QSharedPointer<qrtext::lua::ast::Node> &node);
	bool isArray(SpikePythonType type) const;
	SpikePythonType elementType(SpikePythonType arrayType) const;
	QString newRegister(const QSharedPointer<qrtext::lua::ast::Node> &node);
	QString newRegister(SpikePythonType type);
	...
	QMap<SpikePythonType, int> mArrayDeclarationCount;
	QMap<qReal::Id, QMap<SpikePythonType, int>> mRegistersCount;
