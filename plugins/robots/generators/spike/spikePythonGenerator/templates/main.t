...
// This is an asm-like code to deal with native SPIKE software. All your nice TRIK Studio
...
subcall atan2wrap_SPIKE_KERNEL_util
{
	IN_F y
	IN_F x
	OUT_F z
...
subcall motors_overflow_check_SPIKE_KERNEL_util
{
	IN_32 src
	OUT_32 dst
...
subcall clp2_SPIKE_KERNEL_util
{
	IN_32 src
	OUT_32 dst
...
subcall write32Array_SPIKE_KERNEL_util
{
	IN_H array
	IN_32 pos
	IN_32 value

	CALL(writeArray_SPIKE_KERNEL_util, array, pos)
	ARRAY_WRITE(array, pos, value)
}

subcall writeFArray_SPIKE_KERNEL_util
{
	IN_H array
	IN_32 pos
	IN_F value
...
subcall writeArray_SPIKE_KERNEL_util
{
	IN_H  array
	IN_32  pos
	...
	DATA32 newSize
	CALL(clp2_SPIKE_KERNEL_util, size, newSize)
	ARRAY(RESIZE, array, newSize)
endLabel:
}

subcall assign32Array_SPIKE_KERNEL_util
{
	IN_H srcArray
	IN_H dstArray
...
subcall B2U32_SPIKE_KERNEL_util
{
	IN_8 src
	OUT_32 dst
